...
mod tests {
    ...
    fn https_and_ssh_normalize_to_same() {
        let https = normalize_remote_url("https://github.com/brynary/arc.git");
        let ssh = normalize_remote_url("git@github.com:brynary/arc.git");
        assert_eq!(https, ssh);
        insta::assert_snapshot!(https, @"github.com/brynary/arc");
    }

    #[test]
    fn ssh_protocol_prefix() {
        let result = normalize_remote_url("ssh://git@github.com/brynary/arc.git");
        insta::assert_snapshot!(result, @"github.com/brynary/arc");
    }

    #[test]
    fn url_without_git_suffix() {
        let result = normalize_remote_url("https://github.com/brynary/arc");
        insta::assert_snapshot!(result, @"github.com/brynary/arc");
    }
