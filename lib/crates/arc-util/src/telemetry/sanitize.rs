...
mod tests {
    ...
    fn simple_command_with_flag() {
        let result = sanitize_command(
            &args(&["arc", "run", "my-workflow.toml", "--no-retro"]),
            "run",
        );
        insta::assert_snapshot!(result, @"arc run VALUE --no-retro");
    }

    #[test]
    fn nested_subcommand() {
        let result = sanitize_command(
            &args(&["/usr/local/bin/arc", "pr", "create", "--title", "Fix bug"]),
            "pr create",
        );
        insta::assert_snapshot!(result, @"arc pr create --title VALUE");
    }

    #[test]
    fn model_list_no_extra_args() {
        let result = sanitize_command(&args(&["arc", "model", "list"]), "model list");
        insta::assert_snapshot!(result, @"arc model list");
    }

    #[test]
    fn flag_with_equals_value() {
        let result = sanitize_command(&args(&["arc", "run", "--level=medium"]), "run");
        insta::assert_snapshot!(result, @"arc run --level=VALUE");
    }

    #[test]
    fn boolean_values_kept() {
        let result = sanitize_command(&args(&["arc", "run", "--verbose", "true"]), "run");
        insta::assert_snapshot!(result, @"arc run --verbose true");
    }

    #[test]
    fn file_paths_redacted() {
        let result = sanitize_command(&args(&["arc", "check", "src/main.rs"]), "check");
        insta::assert_snapshot!(result, @"arc check VALUE");
    }

    #[test]
    fn numeric_values_kept() {
        let result = sanitize_command(&args(&["arc", "run", "--retries", "3"]), "run");
        insta::assert_snapshot!(result, @"arc run --retries 3");
    }

    #[test]
    fn semver_kept() {
        let result = sanitize_command(&args(&["arc", "run", "--min-version", "1.2.3"]), "run");
        insta::assert_snapshot!(result, @"arc run --min-version 1.2.3");
    }

    #[test]
    fn positional_key_equals_value() {
        let result = sanitize_command(&args(&["arc", "run", "env=production"]), "run");
        insta::assert_snapshot!(result, @"arc run env=VALUE");
    }
}
