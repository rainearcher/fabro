---
title: "Failures"
description: "How Arc classifies, retries, and recovers from failures during workflow execution"
---
...
When a node fails, Arc classifies the failure into one of six categories. These classes drive retry decisions, circuit breaker logic, and edge routing.
...
Arc retries failures at two levels: **LLM retries** handle transient API errors inside a single model call, and **node retries** re-execute the entire node handler when the first level isn't enough. These layers are independent — a node retry re-runs the full handler, which gets its own fresh set of LLM retries.
...
If the provider returns a `Retry-After` header, Arc respects it — unless the delay exceeds 60 seconds, in which case the call fails rather than blocking the run.
...
This uses the default backoff (200ms initial, 2x exponential) with the specified number of retries.
...
When a model provider fails with a transient error or quota exhaustion, Arc can automatically switch to a different provider. Configure fallback chains in your [run configuration](/execution/run-configuration):
...
When Anthropic is unavailable, Arc tries Gemini first, then OpenAI. For each fallback provider, Arc selects the closest model by matching required capabilities (tool use, vision, reasoning) and minimizing cost difference.
...
Arc has two independent mechanisms for detecting stuck loops: **node visit limits** that catch workflow-level cycles, and **tool call pattern detection** that catches agent-level repetition.
...
Inside an agent session, Arc monitors the last 10 assistant turns for repeating tool call patterns. It detects patterns of length 1 (same call repeated), 2 (A-B-A-B), or 3 (A-B-C-A-B-C). Every complete group in the window must match for detection to trigger.

When a loop is detected, Arc injects a steering message into the conversation:
...
After each failed node, Arc constructs a **failure signature** — a normalized fingerprint combining the node ID, failure class, and error message:
...
Arc tracks signature counts across the run. When the same signature repeats **3 times** (configurable via `loop_restart_signature_limit`), the run is terminated:
...
When a goal gate is unsatisfied at the exit node, Arc looks for a **retry target** — a node to jump back to for another attempt:
...
Arc runs a background watchdog that monitors event activity. If no events are emitted for longer than the **stall timeout**, the run is canceled. This catches cases where a handler hangs indefinitely without producing errors.
...
A node failure does **not** automatically terminate the run. Arc follows this escalation path:
