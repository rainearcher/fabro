...
pub struct SessionState {
    ...
    pub turns: Vec<arc_types::SessionTurn>,
...
fn resolve_model(model_arg: Option<String>) -> (String, Option<String>) {
    ...
    match arc_llm::catalog::get_model_info(&raw) {
        Some(info) => (info.id, Some(info.provider)),
        None => (raw, None),
    }
}

fn turns_to_messages(turns: &[arc_types::SessionTurn]) -> Vec<arc_llm::types::Message> {
    turns
        .iter()
        .filter_map(|turn| match turn {
            arc_types::SessionTurn::UserTurn(t) => Some(arc_llm::types::Message::user(&t.content)),
            arc_types::SessionTurn::AssistantTurn(t) => {
                Some(arc_llm::types::Message::assistant(&t.content))
            }
            arc_types::SessionTurn::ToolTurn(_) => None,
        })
        .collect()
}
...
fn spawn_generation(store: SessionStore, session_id: uuid::Uuid, dry_run: bool, seq_at_start: u64) {
    tokio::spawn(async move {
        ...
        if dry_run {
            ...
            // Append assistant turn to session
            {
                ...
                if let Some(session) = store.get_mut(&session_id) {
                    session.turns.push(arc_types::SessionTurn::AssistantTurn(
                        arc_types::AssistantTurn {
                            kind: arc_types::AssistantTurnKind::Assistant,
                            content,
                            created_at: now,
                        },
                    ));
        ...
        let mut params = arc_llm::generate::GenerateParams::new(&model_id)
            .messages(messages)
            .max_tokens(4096);
...
pub async fn create_session(
    ...
    Json(req): Json<arc_types::CreateSessionRequest>,
) -> Response {
    ...
    let user_turn = arc_types::SessionTurn::UserTurn(arc_types::UserTurn {
    ...
    (
        StatusCode::CREATED,
        Json(arc_types::CreateSessionResponse {
            id: session_id,
            title,
            model: arc_types::ModelReference { id: model_id },
            created_at: now,
            updated_at: now,
        }),
    )
...
pub async fn retrieve_session(
    ...
    match store.get(&id) {
        Some(session) => (
            StatusCode::OK,
            Json(arc_types::SessionDetail {
                id: session.id,
                title: session.title.clone(),
                model: arc_types::ModelReference {
...
pub async fn send_message(
    ...
    State(state): State<Arc<crate::server::AppState>>,
    Path(id): Path<uuid::Uuid>,
    Json(req): Json<arc_types::SendMessageRequest>,
) -> Response {
    let seq = {
        let mut store = state.sessions.write().expect("session store lock poisoned");
    ...
    (
        StatusCode::ACCEPTED,
        Json(arc_types::SendMessageResponse { accepted: true }),
    )
        .into_response()
}
...
pub async fn list_sessions(
    ...
    let mut items: Vec<arc_types::SessionListItem> = store
        .values()
        .map(|session| {
            let last_message_preview = session
                ...
                .last()
                .map(|t| match t {
                    arc_types::SessionTurn::UserTurn(u) => u.content.clone(),
                    arc_types::SessionTurn::AssistantTurn(a) => a.content.clone(),
                    arc_types::SessionTurn::ToolTurn(_) => String::new(),
                })
                .unwrap_or_default();
    ...
    (
        StatusCode::OK,
        Json(arc_types::PaginatedSessionList {
            data,
            meta: arc_types::PaginationMeta { has_more },
        }),
    )
        .into_response()
}
